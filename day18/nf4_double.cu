...
extern "C" nf4 quantize_nf4(const float *input_h, int n, int block_size_outer, int block_size_inner) {
    ...
    // cudaMallocHost((void**)&absmax_intermediate_h, gridSize*sizeof(float));
    ...
    // quant_state.quant_state.absmax = absmax_intermediate_h;
    ...
    cudaFree(absmax_inner);
    cudaFree(absmax_outer);
