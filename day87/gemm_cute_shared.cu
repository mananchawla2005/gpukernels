...
__global__ void tiled_gemm_kernel(
    ...
    for(int t = 0; t < (K_ + TILE_SIZE - 1) / TILE_SIZE; ++t) {
      int aCol = t * TILE_SIZE + threadIdx.x;
      if(row < M_ && aCol < K_) 
        sA[threadIdx.y][threadIdx.x] = A(row, aCol);
      else 
        sA[threadIdx.y][threadIdx.x] = 0.0f;
  
      int bRow = t * TILE_SIZE + threadIdx.y;
      if(bRow < K_ && col < N_) 
        sB[threadIdx.y][threadIdx.x] = B(bRow, col);
      else 
...
int main() {
  ...
  tiled_gemm_kernel<<<grid, block>>>(tensorA, tensorB, tensorC,
    M, N, K, 1.0f, 0.5f);
